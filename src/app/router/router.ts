...
export class Router {
    ...
    private async _onChanged(state: { route?: string, routeSegments?: Array<any> }) { 
        ...
        for (var i = 0; i < this._routes.length; i++) {
            if (state.route == this._routes[i].path) {
                await this._onRouteMatch(this._routes[i].name, this._routes[i].resolve);
                match = true;
            }
        }
        ...
        if (!match) {
            ...
            for (let i = 0; i < this._routes.length; i++) {
                ...
                if (_currentSegments.length === segments.length) {
                    ...
                    if (match) {                                                
                        await this._routes[i].resolve();
                        this._onRouteMatch(this._routes[i].name, this._routes[i].resolve, routeParams);
    ...
    private async _onRouteMatch(name: string, resolve: {():Promise<void>}, routeParams: any = null) {
        await resolve();
